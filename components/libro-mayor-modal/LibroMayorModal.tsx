    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Libro Mayor General" size="4xl">
            ...
            <div className="overflow-y-auto max-h-[70vh] space-y-2 pr-2">
                {Object.keys(ledger).length > 0 ? Object.values(ledger).map((account: LedgerAccount) => {
                                    ...
                                    <div className="bg-white dark:bg-gray-800/50 p-2 overflow-x-auto">
                                        <table className="min-w-full">
                                            <thead className="bg-gray-50 dark:bg-gray-700/50 text-xs">
                                                <tr>
                                                    <th className="px-4 py-2 text-left font-medium text-black uppercase">Fecha</th>
                                                    <th className="px-4 py-2 text-left font-medium text-black uppercase">Descripción</th>
                                                    <th className="px-4 py-2 text-right font-medium text-black uppercase">Debe</th>
                                                    <th className="px-4 py-2 text-right font-medium text-black uppercase">Haber</th>
                                                    <th className="px-4 py-2 text-right font-medium text-black uppercase">Saldo</th>
                                                </tr>
                                            </thead>
                                            <tbody className="text-black">
                                                {account.entries.map((entry, index) => (
                                                    <tr key={index} className="border-t dark:border-gray-700">
                                                        <td className="px-4 py-2 whitespace-nowrap text-sm text-black">{entry.date}</td>
                                                        <td className="px-4 py-2 text-sm text-black">{entry.description}</td>
                                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm font-mono text-black">{entry.debit > 0 ? formatCurrency(entry.debit) : ''}</td>
                                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm font-mono text-black">{entry.credit > 0 ? formatCurrency(entry.credit) : ''}</td>
                                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm font-mono font-semibold text-black">{formatCurrency(entry.balance)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                            <tfoot className="bg-gray-100 dark:bg-gray-900/50 font-bold text-black">
                                                <tr className="border-t-2 border-gray-300 dark:border-gray-600">
                                                    <td colSpan={2} className="px-4 py-2 text-left text-sm uppercase text-black">Totales Cuenta</td>
                                                    <td className="px-4 py-2 text-right text-sm font-mono text-black">{formatCurrency(account.totalDebit)}</td>
                                                    <td className="px-4 py-2 text-right text-sm font-mono text-black">{formatCurrency(account.totalCredit)}</td>
                                                    <td className="px-4 py-2 text-right text-sm font-mono text-black">{formatCurrency(account.finalBalance)}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
